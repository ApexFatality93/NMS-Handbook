...
# output_json_file = './JSON Files/Refining_Table.json'
...
# for refining_product in root.findall('.//Property[@value="GcRefinerRecipe"]'):
#     data = extract_data(refining_product)
#     is_cooking_product = data.get('Cooking', '')

#     if is_cooking_product == 'false':
#         # Get product info for this refining product
#         refining_product_id = data.get('Result', {}).get('Id', '')
#         product = product_lookup.get(refining_product_id, {})
...
#         ingredient_list = []
#         ingredient_node = refining_product.find('./Property[@name="Ingredients"]')
...
#         refining_data_dict[refining_product_id] = {
#             'ProductID': refining_product_id,
#             'Name': name_id,
...
#     json.dump(refining_data_dict, json_out, indent=4, ensure_ascii=False)

# print(f"✅ Refining data written to: {output_json_file}")
# print(f"🧾 Total refining products: {len(refining_data_dict)}")
...
for refining_product in root.findall('.//Property[@value="GcRefinerRecipe"]'):
    ...
    product = product_lookup.get(product_id) or substance_lookup.get(product_id, {})
    ...
    # Add recipe entry
    refining_data_dict[product_id]['Recipes'].append({
        'RecipeType': lang_lookup.get(data.get('RecipeType', ''), ''),
        'RecipeName': lang_lookup.get(data.get('RecipeName', ''), ''),
        'TimeToMake': data.get('TimeToMake', ''),
...
print(f"✅ Refining data written to: {output_json_file}")
print(f"🧾 Total unique refining products: {len(refining_data_dict)}")